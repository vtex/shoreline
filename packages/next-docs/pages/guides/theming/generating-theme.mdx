...
The `shoreline.config.ts` should export a config definition based on a created [preset](/guides/theming/configuration-presets). You can also [extend our arpoador preset](/guides/theming/generating-theme#extending-shorelines-theme).
...
In order to expand on our existing theme you will have to import the arpoador theme preset and use it as a base preset in your config file.
...
```sh npm2yarn copy
npm i @vtex/shoreline-theme-arpoador
```

### Use imported theme as base in the `shoreline.config.ts` file

In defineConfig use the preset `presetArpoador` and add other properties to overwrite what you would like to change
