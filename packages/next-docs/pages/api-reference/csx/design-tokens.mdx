...
The Csx object represents CSS styles within a typescript object, where the keys represent the css property in camelCase, and the entry - the css-value. One possible value is tokens. Shoreline uses the `$` symbol (dollar sign) to trigger a token transformation. A token pattern is defined by:
