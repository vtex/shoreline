...
When no items have been created or received:

- Hide elements that are unnecessary in this state such as any Search, Filter, and Pagination.
...
When the search works but no results match the terms:

- Keep the Search, Filters, and any Buttons active, and disable both Paginations.
- Replace the Table or any container being used to display items with an EmptyState component.
...
When items fail to load or the search fails to return results:

- Keep the Search and Filters active, and disable both Paginations. Disable any other button that can't be triggered while in this state.
- Replace the Table or any container being used to display items with an EmptyState component.
