...
| Value | Usage |
| --- | --- |
| `primary` | Use only one at a time, for the most frequently triggered action in a container. |
| `secondary` | Use for occasionally triggered actions. |
| `tertiary` | Use for actions that are rarely triggered actions or that exist multiple times in a Page. |
| `critical` or `criticalTertiary` | Use for irreversible actions. Prefer to keep actions reversible and not use this. |

### Size

| Value | Usage |
| --- | --- |
| `large` | Use in the header or footer of a container when its width is 800px or larger. |
| `normal` | Use in most scenarios. |

## Position

- **Position in a container:** In most scenarios, actions should be listed horizontally, right aligned and vertically centered. In a form, left-align actions that modify content and right-align those that submit content.
...
- Use an Icon Button if the the label of an action should not be visible.