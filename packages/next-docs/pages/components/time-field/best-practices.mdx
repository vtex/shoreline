...
- Mark a Time Field as optional when it's sometimes necessary.
...
- **How to write:** Write a single sentence that is short and direct. Use sentence case, but capitalize proper nouns. Don’t use periods, commas, or include redundant words.
    - **Explaining the label:** Write as if completing the sentence “*This is the…”*. For example, write for a *Publish time* field the text "*Time when the promotion will be published to the store*".
    - **Explaining how the value will be used:** Write as if completing the sentence “*When you fill a value in this field, it…”* and start with a verb in the simple present tense. For example, write for a *Publish time* field the text "*Considers the time zone where the store is located*”.
