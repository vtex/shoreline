...
- Mark a Date Picker as optional when it's sometimes necessary.
...
Include the time mask beside the date mask in the Date Picker when the merchant can fill a specific time when picking a date.

### Label

- Write the name that best describes the value that must be filled. For example, use *Publish date* when the value should be the date when a release should be published.
...
- Don't include redundant words. For example, write *Publish date* as the label instead of *Release publish date* on a page that includes Release in its title.
...
- **How to write:** Write a single sentence that is short and direct. Use sentence case, but capitalize proper nouns. Don’t use periods, commas, or include redundant words.
    - **Explaining the label:** Write as if completing the sentence “*This is the…”*. For example, write for a *Publish date* field the text "*Day in which the changes made will appear in the store*".
    - **Explaining how the value will be used:** Write as if completing the sentence “*When you fill a value in this field, it…”* and start with a verb in the simple present tense. For example, write for a *Publish date* field the text "*Considers the time zone where the store is located*”.
    - **Explaining the label and how the value will be used:** Combine the two sentences described in the previous topics and adapt them if necessary. For example, “*Day in which the changes made will be published considering the time zone where the store is located*".
