...
- Don't include redundant words. For example, write *Name* as the label instead of *Promotion name* on a page that includes _Promotion_ in its title.
...
- **How to write:** Write a single sentence that is short and direct. Use sentence case, but capitalize proper nouns. Don’t use periods, commas, or include redundant words.
  - **Explaining the label:** Write as if completing the sentence “_This is the…”_. For example, write for a _Seller ID_ field the text "_Seller identifier in the marketplace_".
  - **Explaining how the value will be used:** Write as if completing the sentence “_When you fill a value in this field, it…”_ and start with a verb in the simple present tense. For example, write for a _Seller ID_ field the text "_Appears to customers in the store_”.
  - **Explaining the label and how the value will be used:** Combine the two sentences described in the previous topics and adapt them if necessary. For example, “_Seller identifier in the marketplace that appears to customers in the store_".
...
- When the field is in an error state, always include an error text. Start with an imperative verb (e.g. _Fill this information_ instead of _This field is required),_ don't use incomplete or imprecise sentences (e.g. _Fill only letters_ instead of _Fill only valid characters)_, and don’t use periods or commas.
