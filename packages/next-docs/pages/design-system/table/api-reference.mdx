...
export type BaseResolver<T> = {
   ...
   * context: Relevant global information about the table current state
   */
  render: ({
    item: T,
    data: unknown,
...
It also accepts all props of `table` html element.

### columns

- Type: `Array<TableColumn<T, BaseResolvers<T>>>`
...
It accepts all props of `thead` html element.

## THeadCell optional props

It also accepts all props of `div` html element.
...
It accepts all props of `tbody` html element.

## TableBodyRow optional props

It also accepts all props of `tr` html element.
