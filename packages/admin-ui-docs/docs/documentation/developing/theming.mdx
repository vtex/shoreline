...
When you override the default theme and you still want to style native JSX elements, instead of using the [**csx** function](theming#csx), as shown earlier, you should use the [**cn** function](hooks#cn), because the `csx` function only uses the default theme.
...
```jsx live
<h1 className={csx({ text: '$pageTitle' })}>
  Page title
</h1>
```
