...
<PageSection
  ...
  title={tabToc.overview[5].value}
  explanation="You can persist the search value within the browser's query params using the `useQuerySearchState` hook, instead of `useSearchState`. The application should be wrapped by `QueryStateProvider`. Watch how your url changes (Note that the persisted value is the debounce value)."
>
...
| onSubmit         | `FormEventHandler<'form'>` | Action to take on submit the form | 🚫       | -       |
