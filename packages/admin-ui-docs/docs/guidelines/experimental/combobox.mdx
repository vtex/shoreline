...
a `renderOption` function which should return a ReactNode that represents this object visually. If you don't define these 2 functions and you use objects on your list you will get unexpected behaviour.
...
#### Custom rendering for i18n
  ...
  You can use custom item rendering as a means of internationalization for each option.
...
The hooks returns `ComboboxMultipleState`, which extends `ComboboxState` and additionally has:

| Name           | Type                       | Description             |
| -------------- | -------------------------- | ----------------------- |
| selectedItems       | `ItemType[]`                 | List of selected items  |
| checkboxState   | Ariakit's CheckboxState   | Used internally for managing checkboxes
...
| renderTag | `(item: T) => ReactNode`   | Same as the one inputed to hook        |
