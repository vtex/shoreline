...
The use of `React.FunctionComponent` (or the `React.FC` shorthand) is discouraged to type the component props. If you need to type children explicitly, use `React.ReactNode`. Find out [some good reasons why](https://github.com/facebook/create-react-app/pull/8177).
...
A deep directory nesting can cause a lot of pain points, so you must avoid it as much as you can.[Learn why here](https://reactjs.org/docs/faq-structure.html#avoid-too-much-nesting).
