...
This property specifies the name of an input, and it is used to subscribe it. You can pass this prop to every component in this library.
...
Is the ability to watch to individual inputs and formState updates without re-rendering the entire form. There are two ways of subscribing to a input element: by using the `Watch` component or the `useWatch` hook.
...
This hook is what it makes possible to subscribe to each form input, and isolate the re-render at the hook level.
It has its scope in terms of subscriptions, so it would not affect other `useFormState`.
...
Extends all props of `TextInput` Admin UI element.
...
Extends all props of `NumberInput` Admin UI element.
...
Extends all props of `Select` Admin UI element.
...
Extends all props of `Radio` Admin UI element.
...
Extends all props of `RadioGroup` Admin UI element.
...
Extends all props of `Checkbox` Admin UI element.
...
Extends all props of `CheckboxGroup` Admin UI element.
...
Extends all props of `Switch` Admin UI element.
...
React Hook to subscribe to input changes that returns the input value state.
...
React Component to subscribe to input changes that renders the input value state.
...
The values returned by the `useFormState` hook.
...
| validate         | `Function, Object` | You can pass a callback function as the argument to validate, or you can pass an object of callback functions to validate all of them. This function will be executed on its own without depending on other validation rules include required attribute |
