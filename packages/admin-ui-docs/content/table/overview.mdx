...
const columns = createColumns([
  ...
  },
  {
    ...
    resolver: {
      type: 'root',
      render: ({ item }) => {
        return <Box csx={{ minWidth: '6rem' }}>{item.brand}</Box>
      },
    },
  },
  {
...
function FilterControls() {
  ...
  useEffect(() => {
    const filtered = items.filter((item) => {
      if (statusState.appliedItem && statusState.appliedItem.label !== item.status) {
        return false
      }
...
const columns = createColumns([
  ...
  },
  {
    ...
    resolver: {
      type: 'root',
      render: ({ item }) => {
        return <Box csx={{ minWidth: '10rem' }}>{item.label}</Box>
      },
    },
  },
  {
