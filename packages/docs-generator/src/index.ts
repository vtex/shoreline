...
import { defaultTypedocConfig, paths, tokens, typedocTokens } from './config'
...
class AutogeneratedDocs {
  constructor(props: {
    ...
    for (const pkg of packages) {
      const { paths, config } = pkg

      this.generateDocs(paths, config, rootPath)
    }
  }
    ...
    for (const fileContents of rootFileContent) {
      try {
        ...
        const methodName = content
          ...
          .find((line: string) => line.startsWith(typedocTokens.methodDocStart))
          ?.replace(typedocTokens.methodDocStart, tokens.empty)
          .trim()
        ...
        if (methodName) {
          ...
          const isTypeAlias = methodName.includes(typedocTokens.props)
