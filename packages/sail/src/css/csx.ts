...
import { getMixin, isMixin } from './mixin'
...
function cssVar(prop: string, value: string) {
  const namespace = get(namespaces, prop)

  if (!namespace) {
    return value
  }

  return `var(--aui-${namespace}-${value}, ${value})`
}

function parseStyle(csxObject: any) {
  const cssObject: any = {}

  for (const key in csxObject) {
    const cssProperty = alias(key)
    const cssValue = csxObject[key]

    if (cssValue && typeof cssValue === 'object') {
      cssObject[cssProperty] = parseStyle(cssValue)
      continue
    }

    if (isMixin(cssProperty)) {
      const mixin = getMixin(cssProperty, cssValue)

      Object.assign(cssObject, mixin)
    } else {
      cssObject[cssProperty] = cssVar(cssProperty, cssValue)
    }
  }
...
export function csx(csxObject: any) {
  const cssObject = parseStyle(csxObject)
