...
* the variant property values has changed from `'info' | 'positive' | 'warning' | 'critical'` to ` 'informational' | 'success' | 'critical' | 'warning'`
...
* the `variant` property values has changed from `primary | secondary | tertiary | critical |criticalSecondary |criticalTertiary | neutralTertiary` to `'primary' | 'secondary' | 'tertiary' | 'critical' | 'criticalTertiary'`
...
You can check their documentation for detailed information.
