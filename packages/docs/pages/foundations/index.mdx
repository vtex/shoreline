# Design Tokens

Shoreline design tokens include Color, Typography, Spacing, Border, Border Radius, and Elevation. Want to know more about design tokens? Read the [documentation](https://shoreline.vtex.com/guides/styling/design-tokens).

## Color

Reflects a product's style, creates hierarchy, and provides meaning to components and messages.

### Base

<br />

<TokensGrid foundation="color" />

### Background

<br />

<TokensGrid foundation="bg" />

### Foreground

<br />

<TokensGrid foundation="fg" />

## Typography

Creates hierarchies, organizes information, and guides users through a product or experience.

<br />

<TokensGrid foundation="typography" />

## Spacing

Defines the relationship between elements and makes the content more easily scannable.

<br />

<TokensGrid foundation="space" />

## Border

Defines the limits of a container and helps organize content.

<br />

<TokensGrid foundation="border" />

## Border Radius

Defines the roundness of the corners of an element.

<br />

<TokensGrid foundation="border-radius" />

## Elevation

Creates spatial relationships by lifting elements from the background.

<br />

<TokensGrid foundation="elevation" />
