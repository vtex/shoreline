...
Spacing tokens are identified in Shoreline by the prefix `$space`. They can be followed by a suffix that is a number which is the space value in rems (or the value in pixels if multiplied by 4), for example the token for a 16px space is `$space-4`, or by the suffix `gap`, which is the default spacing between elements in a page.
...
| `$space-4`       | - The value of `$space-gap` <br/>- Content paddings in containers 120 - 239px <br/><br/>**Vertical:** <br/>- Controls in a group (Checkbox Group or Radio Group) <br/>- Content padding-top in Pages without tabs in screens 380 - 559px |
