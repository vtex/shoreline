...
The palette is fixed and never cycled: the first series always wears the primary color, the second the secondary, and so on. Red is deliberately absent — it carries error semantics elsewhere in Shoreline.
...
| `pink-9` | Extended, `maxSeries` ≥ 4 |
| `teal-9` | Extended, `maxSeries` ≥ 5 |
| `green-9` | Extended, `maxSeries` = 6 |
