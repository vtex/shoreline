...
class ComponentDocGenerator {
      ...
      try {
        ...
        if (componentDocs && Array.isArray(componentDocs)) {
          ...
          componentDocs.forEach((componentDoc, index) => {
            ...
            docs[componentName] = this.formatComponentDoc(componentDoc);
            processedCount++;
          });
        } else if (componentDocs) {
          // Single component
          const componentName = componentDocs.displayName || this.getComponentName(file);
          docs[componentName] = this.formatComponentDoc(componentDocs);
          processedCount++;
        } else {
          skippedCount++;
          if (this.verbose) {
  ...
  formatComponentDoc(componentDoc) {
    ...
    return {
      ...
      props: Object.entries(componentDoc.props || {}).reduce((acc, [propName, propData]) => {
        ...
        acc[propName] = {
          ...
          description: (propData.description || '').replace(/\s+/g, ' ').trim(),
